describe('Create and connect to an account', () => {
    it('Visits the Oc commerce site', () => {
      ...
      // cy.contains('fname')
      cy.get('[id^=fname]').type('fakeuser1')
      cy.get('[id^=lname]').type('totoo')
      cy.get('[id^=username]').type('fakeuser1')
      cy.get('[id^=email]').type('fake1@email.com')
      cy.get('[id^=pass]').type('11hstesh<23456789')
      cy.get('[id^=re_pass]').type('11hstesh<23456789')
